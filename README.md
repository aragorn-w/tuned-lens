...
| File | Description |
|------|-------------|
| `tuned_lens.py` | Core implementation: `TunedLens` module (from-scratch affine probes), training loop with KL divergence loss, terminal heatmap visualization, CLI via typer |
| `test_tuned_lens.py` | Test suite (56 tests): color mapping, token sanitization, inference, module internals, rendering, training pipeline |
| `tuned_lens_weights/` | Pre-trained probe weights (`tuned_lens_probes.pt`) and training loss curves (`loss_curves.png`) |
...
**Quick test run** (~2 minutes, 1 epoch on 50 samples with shorter sequences):

```bash
uv run python tuned_lens.py train --quick --out ./tuned_lens_weights
```

Pre-trained weights are already included in `tuned_lens_weights/`, so retraining is not required to run visualizations.
