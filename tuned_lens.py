...
def train(
    ...
    quick: bool = typer.Option(
        False, "--quick",
        help="Fast test run: 1 epoch, 50 samples, 128 max length (~2 min).",
    ),
) -> None:
    """Train tuned lens probes on The Pile (10k) dataset."""
    if quick:
        # Override settings for a fast end-to-end test
        epochs = 1
        num_samples = 50
        max_length = 128
        console.print("[bold yellow]Quick mode: 1 epoch, 50 samples, "
                      "128 max length[/bold yellow]")
    train_tuned_lens(
        model_name=model,
        out_dir=out,
        epochs=epochs,
